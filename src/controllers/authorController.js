...
exports.author_create_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { return next(err);}
        res.render('author_form', { title: 'Criar Autor', author: results.authors});
    });
};
...
exports.author_create_post = [
    ...
    body('first_name').trim().isLength({ min: 1 }).escape().withMessage('Primeiro nome deve ser preenchido.'),
    body('family_name').trim().isLength({ min: 1 }).escape().withMessage('Segundo nome deve ser preenchido'),
    ...
    async.parallel({
        ...
        if (results.authors_books.length > 0) {
            ...
            Author.findByIdAndRemove(req.body.authorid, function deleteAuthor(err) {
                ...
                req.flash('success', 'Autor deletado');
                res.redirect('/authors');
            });
        }
    });
